...
def load_data(filename):
    ...
    company_DATA = pd.read_csv(filename, encoding="utf8")

    # company_DATA = company_DATA.drop_duplicates()
    print(company_DATA.shape)

    company_DATA = company_DATA[company_DATA.businessScope.notnull(
    ) & company_DATA.industry.notnull()]
    labels = pd.DataFrame.from_dict(getLabels())
    company_DATA = company_DATA.merge(
        labels, left_on="industry", right_on="secnduName")
    company_DATA = company_DATA[company_DATA.secInduCode.notnull()]
    company_DATA["businessScope_Cut"] = company_DATA["businessScope"].apply(
        cut)

    X = company_DATA["businessScope_Cut"]
    y = company_DATA["secInduCode"]
...
def data_iterator(
    ...
    assert total_processed_examples == len(data_X), 'Expected {} and processed {}'.format(
        len(data_X), total_processed_examples)
